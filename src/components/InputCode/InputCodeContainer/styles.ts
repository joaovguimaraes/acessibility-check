...
export const Code = styled.TouchableOpacity.attrs(_ => {
  ...
  ${props => {
    ...
    return `
      ...
      background-color: ${theme.alias.color.elements.bgColor02};

      opacity: ${_getOpacity({...theme, pressed})};
...
function _getOpacity({globals, pressed}: FProps) {
  if (pressed) {
    return globals.shapes.opacity.superHigh;
  }
