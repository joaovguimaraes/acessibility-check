...
export function DsInputCodeContainer({
  label,
  itemsCount = 3,
  hyperlink,
  ...
  function _getCode(index: number): React.ReactElement {
    ...
    if (secureTextEntry && code[index]) {
      return <S.Dot testID="input-code-container.dot" />;
    }
    ...
    <S.Container
      ...
      <S.Content>
        {itemList.map((_, index) => {
          return (
            <S.Code
              testID={'input-code-container.code-' + index}
              key={index}
      ...
      <S.InputView >
        <TextInput
          testID={'input-code-container.input'}
          ref={inputRef}
          value={code}
