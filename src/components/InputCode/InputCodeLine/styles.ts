...
export const Code = styled.TouchableOpacity.attrs(_ => {
  ...
  ${props => {
    const {theme, pressed, selected} = props;

    return `
      width: ${theme.globals.shapes.size.s5x}px;
      height: ${theme.globals.shapes.size.s7x}px;
      background-color: ${getBackgroundColor({...theme, pressed})};

      border-radius: ${theme.globals.shapes.border.radiusNone}px;
      border-bottom-width: ${theme.alias.defaultt.borderWidth}px;
      border-color: ${getBorderColor({...theme, selected})};
    `;
  }}
`;
...
function getBackgroundColor({alias, pressed}: FProps) {
  if (pressed) {
...
function getBorderColor({alias, selected}: FProps) {
  if (selected) {
    return alias.color.active.placeholderColor;
  }

  return alias.color.elements.bordercolor;
}
